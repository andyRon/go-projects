...
func WebList(c echo.Context) error {
	var (
		...
		wb       = make([]*models.Web, 0)
		cnt      int64
	)
	size = helper.If(size == 0, define.PageSize, size).(int)
	index = helper.If(index == 0, 1, index).(int)
	err := models.DB.Model(new(models.Web)).Count(&cnt).Offset((index - 1) * size).Limit(size).Find(&wb).Error
	if err != nil {
...
func WebAdd(c echo.Context) error {
	...
	// 判断域名是否已存在
	err := models.DB.Model(new(models.Web)).Where("domain = ?", domain).Count(&cnt).Error
	if err != nil {
		log.Println("[DB ERROR] : " + err.Error())
		return c.JSON(http.StatusOK, echo.Map{
			"code": -1,
...
func WebEdit(c echo.Context) error {
	...
	// 判断域名是否已存在
	err := models.DB.Model(new(models.Web)).Where("domain = ? AND identity != ?", domain, identity).Count(&cnt).Error
	if err != nil {
		log.Println("[DB ERROR] : " + err.Error())
		return c.JSON(http.StatusOK, echo.Map{
			"code": -1,
...
func WebDelete(c echo.Context) error {
	...
	err := models.DB.Where("identity = ?", identity).Delete(new(models.Web)).Error
	if err != nil {
		log.Println("[DB ERROR] : " + err.Error())
		return c.JSON(http.StatusOK, echo.Map{
			"code": -1,
