...
func SetRoleUpdateAdmin(c *gin.Context) {
	...
	err = models.DB.Model(new(models.Role)).Where("identity = ?", in.Identity).
...
func SetRoleDelete(c *gin.Context) {
	...
	// 判断当前角色是否已关联用户
	var cnt int64
	err := models.DB.Model(new(models.User)).Where("role_identity = ?", identity).Count(&cnt).Error
	if err != nil {
		helper.Error("[DB ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
	...
	// 删除角色
	err = models.DB.Where("identity = ?", identity).Delete(new(models.Role)).Error
	if err != nil {
		helper.Error("[DB ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
...
func SetRoleUpdate(c *gin.Context) {
	...
	// 角色ID
	err = models.DB.Model(new(models.Role)).Select("id").
	...
	err = models.DB.Transaction(func(tx *gorm.DB) error {
		// 更新角色
		err = tx.Model(new(models.Role)).Where("id = ?", rb.ID).Updates(map[string]interface{}{
			"name":     in.Name,
			"is_admin": in.IsAdmin,
			"sort":     in.Sort,
		}).Error
...
func SetRoleDetail(c *gin.Context) {
	...
	// 1. 获取角色基本信息
	rb, err := models.GetRole(identity)
	if err != nil {
		helper.Error("[DB ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
