...
func UserPasswordChange(c *gin.Context) {
	...
	// 判断旧密码是否正确
	var cnt int64
	err = models.DB.Model(new(models.User)).
	...
	// 修改密码
	err = models.DB.Model(new(models.User)).Where("identity = ?", userClaim.Identity).
...
func SetUserAdd(c *gin.Context) {
	...
	// 1. 判断用户名是否存在
	var cnt int64
	err = models.DB.Model(new(models.User)).Where("username = ?", in.Username).Count(&cnt).Error
	if err != nil {
		helper.Error("[DB ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
...
func SetUserUpdate(c *gin.Context) {
	...
	// 1. 判断用户名是否已存在
	var cnt int64
	err = models.DB.Model(new(models.User)).Where("identity != ? AND username = ?", in.Identity, in.Username).Count(&cnt).Error
	if err != nil {
		helper.Error("[DB ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
	...
	// 2. 修改数据
	err = models.DB.Model(new(models.User)).Where("identity = ?", in.Identity).Updates(map[string]any{
...
func SetUserDelete(c *gin.Context) {
	...
	// 删除管理员
	err := models.DB.Where("identity = ?", identity).Delete(new(models.User)).Error
	if err != nil {
		helper.Error("[DB ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
