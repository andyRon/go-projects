...
func DevMenuAdd(c *gin.Context) {
	...
	if in.ParentIdentity != "" {
		err = models.DB.Model(new(models.Menu)).Select("id").
	...
	// 2 保存数据
	err = models.DB.Create(&models.Menu{
...
func DevMenuUpdate(c *gin.Context) {
	...
	// 2 更新数据
	err = models.DB.Model(new(models.Menu)).Where("identity = ?", in.Identity).Updates(map[string]interface{}{
		"parent_id": parentId,
		"name":      in.Name,
		"web_icon":  in.WebIcon,
		"sort":      in.Sort,
...
func DevMenuDelete(c *gin.Context) {
	...
	// 1. 删除数据库中的数据
	err := models.DB.Where("identity = ?", identity).Delete(new(models.Menu)).Error
	if err != nil {
		helper.Error("[DB ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
...
func DevFuncAdd(c *gin.Context) {
	...
	// 2. 保存数据
	err = models.DB.Create(&models.Function{
		Identity: helper.UUID(),
		MenuId:   menuId,
		Name:     in.Name,
		URI:      in.Uri,
...
func DevFuncUpdate(c *gin.Context) {
	...
	// 2. 更新数据
	err = models.DB.Model(new(models.Function)).Where("identity = ?", in.Identity).Updates(map[string]interface{}{
		"menu_id": menuId,
		"name":    in.Name,
		"uri":     in.Uri,
		"sort":    in.Sort,
...
func DevFuncDelete(c *gin.Context) {
	...
	err := models.DB.Where("identity = ?", identity).Delete(new(models.Function)).Error
	if err != nil {
		helper.Error("[DB ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
