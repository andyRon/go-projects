	...
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			if err != nil {
				...
				err = models.DB.Model(new(models.Role)).Select("is_admin").Where("identity = ?", userClaim.RoleIdentity).Find(role).Error
				if err != nil {
				...
				// 同步 Redis，默认保存一周
				models.RDB.Set(context.Background(), adminRoleKey, role.IsAdmin, time.Second*3600*24*7)
				if role.IsAdmin == 1 {
					isAdmin = "1"
				} else {
					isAdmin = "0"
	...
	return func(c *gin.Context) {
		...
		if !userClaim.IsAdmin {
			...
			if keyExist > 0 {
				...
				// key不存在，从DB中查询数据，并保存
				data, err := models.GetAuthFunctionURI(userClaim.RoleIdentity)
				if err != nil {
					helper.Error("[DB ERROR] : %v", err)
					c.Abort()
					c.JSON(http.StatusOK, gin.H{
